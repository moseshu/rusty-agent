...
//!
//! Three consumers need this answer and none of them can read sentences: R17-3 routes graph edges
//! on it, R15 decides whether a closeout step is still owed, and R13 renders it to the host. Every
//! one of those would otherwise have to re-derive the reason from the last message, which is the
//! text-driven control flow R7-10 forbids.
//!
...
/// the model concluded, a budget ran out (R3-8), `tool_use_behavior` stopped the loop (R3-5), or an
/// error handler produced the output (R3-8). They demand different responses, so they are different
/// values.
///
...
/// rule 5). This is not one: R17-3 edges route on these values, so a free-form reason would put
...
pub enum FinishReason {
    ...
    Final,
    /// A tool result became the final output under `tool_use_behavior` (R3-5).
    ToolStop,
    /// The turn limit was reached before the model concluded.
    MaxTurns,
    /// A token, cost, or wall-clock budget ran out and the run ended softly (R3-8).
    BudgetExhausted,
    /// The run was cancelled: a user interrupt, a shutdown, or an expired deadline.
    Cancelled,
    /// An error handler produced the outcome in place of the model (R3-8).
    ErrorHandled,
    /// A guardrail tripped and stopped the run (R7).
    GuardrailTripped,
}
...
impl FinishReason {
    ...
    ///
    /// R3-8 has to make exactly this call when a budget ends a run softly. Pinning the mapping here
    /// keeps every caller from re-deriving it, and differently.
    #[must_use]
    ...
    ///
    /// R15 asks this to decide whether a closeout step is still owed, and R17-3 asks it to pick
    /// between a success edge and a fallback edge. The distinction is **who ended the run**, not
    /// whether the answer was good: a model that concludes wrongly still finished.
    #[must_use]
    ...
    ///
    /// R6-6 resume and R13's "continue" affordance ask this. It is narrower than
