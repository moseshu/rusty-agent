...
use crate::budget::{opaque_block_bytes, source_text};
...
impl ToolOutputTrimmer {
    ...
    fn trim_output(&self, output: &Value, tool_name: &str) -> Result<Option<Value>> {
        let Some(structured) = ToolOutput::from_stored(output)? else {
            ...
            return Ok((source.output_chars > self.max_output_chars).then(|| {
                Value::String(self.summary_text(tool_name, &source, self.max_output_chars))
            }));
        };
        ...
        let Some(summary_budget) = structured_summary_budget(&structured, self.max_output_chars)?
        else {
        ...
        let trimmed = trimmed_structured_output(&structured, summary)?;
...
/// complete record survives this second projection; a *new* reference cannot be minted here,
/// because one names a run and this projection is not given one.
fn trimmed_structured_output(output: &ToolOutput, summary: String) -> Result<ToolOutput> {
    let trimmed = ToolOutput::text(summary.clone()).with_metadata(output.metadata().clone());
    match output.model_excerpt() {
        Some(excerpt) => Ok(trimmed.with_model_excerpt(ModelExcerpt::new(
            vec![ToolOutputBlock::text(summary)],
            excerpt.artifact_ref().clone(),
        )?)),
        None => Ok(trimmed),
    }
}
...
fn structured_summary_budget(
    ...
    let fixed = trimmed_structured_output(output, String::new())?;
